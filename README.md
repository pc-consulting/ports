...
- finance/py-tryton*, finance/tryton-*
    - General observations, unless noted more specifically below:
        ...
        - portlint passes, poudriere testport passes, tryton.cfg dependencies should be in Makefiles
        - Otherwise still untested
    ...
    - Individual ports, as currently are:
        - finance/tryton-server
            ...
            - ToDos:
                - Create & install sample configuration (trytond can read multiple configs so one /usr/local/etc/trytond.cfg plus several ones in ...etc/tryton/ could be an idea)
                - Create & install startup scripts
                - Have all installed with a specific user id, e. g. www, or tryton (which I happen to like better, maybe wit gid=www or so)
        - finance/py-tryton
